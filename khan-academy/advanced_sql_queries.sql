...
QUERIES ARE AFTER THE DATA --- LINE 57 ---
...
SELECT body, AVG(mean_radius) FROM solar_system_objects;

SELECT body, MAX(surface_gravity_rel) FROM solar_system_objects;

SELECT body,
    CASE
        WHEN surface_gravity_rel > 0 AND surface_gravity_rel < 2 THEN "Twice the Gravity of Earth"
        WHEN surface_gravity_rel >= 2 AND surface_gravity_rel < 3  THEN "Tripple the Gravity of Earth"
        ELSE "DOA"
    END AS Relative_Gravity
FROM solar_system_objects;

SELECT type_of_object, COUNT(type_of_object) FROM solar_system_objects GROUP BY type_of_object;

SELECT type_of_object, COUNT() AS num_objects_w_mass_gtr_ten FROM solar_system_objects GROUP BY type_of_object HAVING ROUND(mass) > 10;
